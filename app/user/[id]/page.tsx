...
export default function UserProfilePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        addDebugLog(`사용자 프로필 로딩 시작: ${params.id}`)
  ...
  const fetchSolvedChallenges = async (userId: string, profileData: UserProfile | null) => {
    ...
    try {
      ...
      try {
        ...
        ctfResultsSnapshot.forEach((doc) => {
          const data = doc.data()
      ...
      try {
        ...
        wargameChallengesSnapshot.forEach((doc) => {
          ...
          if (data.solvedBy && Array.isArray(data.solvedBy)) {
            const found = data.solvedBy.find((solver: any) => {
            ...
            if (found) {
              isSolved = true
              if (typeof found === "object" && found.solvedAt) {
                solvedAt = found.solvedAt
              }
            }
          }

          if (!isSolved && data.solvedTimes && data.solvedTimes[userId]) {
            isSolved = true
      ...
      try {
        ...
        let foundInCtf = 0
        ctfProblemsSnapshot.forEach((doc) => {
          const data = doc.data()
          let isSolved = false
          let solvedAt = Timestamp.now()
  ...
  const refreshData = async () => {
    ...
    try {
      await fetchSolvedChallenges(params.id, profile)
      toast({
        title: "데이터 새로고침 완료",
